...
Mahasiswa *rightRotate(Mahasiswa *curr){
	...
	curr->height = 1 + max(getHeight(curr->left), getHeight(currLeft->right));
	currLeft->height = 1 + max(getHeight(currLeft->right), getHeight(currLeft->right));
...
Mahasiswa *leftRotate(Mahasiswa *curr){
	...
	currRight->height = 1 + max(getHeight(currRight->left), getHeight(currRight->left));
...
void updateTree(Mahasiswa *root, int age, char newName[]){
	...
	else if(age < root->age) updateTree(root->left, age, newName);

	else if(age > root->age) updateTree(root->right, age, newName);
